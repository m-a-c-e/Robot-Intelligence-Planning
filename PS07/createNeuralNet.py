...
def createNeuralNet(numNodesPerLayer, numInputDims, numOutputDims):
    ...
    for i in range(numLayers + 1):
        if i == 0:
            ...
            nn.append([np.random.random((numNodesPerLayer[i], numInputDims)) * np.sqrt(2 / numInputDims), np.zeros((numNodesPerLayer[i], 1))])
        elif i == numLayers:
            # Use numOutputDims for the output size
            nn.append([np.random.random((numOutputDims, numNodesPerLayer[i-1])) * np.sqrt(2 / numNodesPerLayer[i-1]), np.zeros((numOutputDims, 1))])
        else:
            nn.append([np.random.random((numNodesPerLayer[i], numNodesPerLayer[i-1])) * np.sqrt(2 / numNodesPerLayer[i-1]), np.zeros((numNodesPerLayer[i], 1))])
    return nn
